...
describe('HeaderBody Component', () => {
  ...
  it('renders the logo and brand navigation', () => {
    renderWithIntl(<HeaderBody {...defaultProps} />);
  ...
  it('renders course lockup information', () => {
    ...
    const courseOrgNumber = screen.getByText('EDX 101');

    expect(courseTitle).toBeInTheDocument();
    expect(courseOrgNumber).toBeInTheDocument();
  });

  it('calls onNavigate when course lockup is clicked', () => {
    renderWithIntl(<HeaderBody {...defaultProps} />);

    const courseLockUpLink = screen.getByText('Test Course');
    fireEvent.click(courseLockUpLink);

    expect(mockOnNavigate).toHaveBeenCalledWith('/courses/edx/course-101');
  });
